...
static const float COMBO_FINISHER_DAMAGE = 8.0f; // 3rd combo hit (vs 4.0 for hits 1-2)
...
void character_update(void)
{
    ...
    else if (characterState == CHAR_STATE_ATTACKING || characterState == CHAR_STATE_ATTACKING_STRONG)
    {
        ...
        // Third hit in the combo chain is the finisher and deals extra damage.
        if (characterState == CHAR_STATE_ATTACKING && attackComboIndex == 3) {
            damage = COMBO_FINISHER_DAMAGE;
        }

        if (actionTimer > hitStart && actionTimer < hitEnd) {
            if (!character.currentAttackHasHit && player_weapon_hit_boss()) {
                ...
                if (boss) {
                    ...
                    if (character_sword_world_segment(swordBase, swordTip)) {
                        ...
                        fx_blood_burst(impact[0], impact[1], impact[2], strength);
                    }
                }
