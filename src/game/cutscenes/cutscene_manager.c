...
void cutscene_manager_reset(void)
{
    ...
    // Rewind the one-shot chain-break animation so PHASE1_BREAK_CHAINS plays from
    // the start on a fight restart instead of staying frozen on its final frame.
    if (cutsceneChainBreakAnimations && cutsceneChainBreakAnimations[0]) {
        t3d_anim_set_time(cutsceneChainBreakAnimations[0], 0.0f);
        t3d_anim_set_playing(cutsceneChainBreakAnimations[0], true);
    }
}
